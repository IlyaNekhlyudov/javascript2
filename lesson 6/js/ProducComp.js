Vue.component('products', {
    data(){
        return {
            ...
            // searchAPI: this.$root.$refs.search, почему-то не работает 
        }
    },
    mounted(){
    ...
    template: `
        <div class="products">
            <product v-for="item of this.$root.$refs.search.filtered" :key="item.id_product" :img="imgCatalog" :product="item"></product>
            <p v-if='this.$root.$refs.search.filtered.length === 0' class='no-products'>Товаров не найдено.</p>
        </div>
    `
});
